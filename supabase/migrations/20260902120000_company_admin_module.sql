...
select c.id, v.service, true
from public.companies c
cross join (values ('lunch'), ('dinner')) as v(service)
...
where loc.company_id is null
  and (
    lower(loc.display_name) = lower(c.name)
    or lower(loc.slug) = lower(c.slug)
    or lower(loc.code) = upper(c.slug)
    or lower(coalesce(loc.display_name, '')) like lower(c.name) || '%'
  );
...
begin
  if tg_op = 'INSERT'
    or coalesce(new.company_slug, '') is distinct from coalesce(old.company_slug, '')
  then
    v_company_slug := nullif(trim(coalesce(new.company_slug, '')), '');
    if v_company_slug is null then
      ...
      where (
        ...
        new.order_location_id is null
        and nullif(trim(coalesce(new.location, new.requesting_location_code, '')), '') is not null
    ...
    perform public.assert_company_order_allowed(v_company_slug);
  end if;
  return new;
end;
$$;
...
begin
  ...
  where v_is_admin
    ...
    or (c.active = true and c.visibility = 'public');
...
begin
  ...
  if nullif(p_company->>'id', '') is not null and v_slug is not null then
    select c.slug
    into v_existing_slug
    from public.companies c
    where c.id::text = p_company->>'id'
    limit 1;

    if found and v_existing_slug <> v_slug then
...
begin
  ...
  if nullif(p_company->>'id', '') is not null then
    ...
    if found and v_existing_company.slug <> v_slug then
  ...
  set name = excluded.name,
      ...
      options_source_slug = excluded.options_source_slug,
      settings = excluded.settings,
      label_settings = excluded.label_settings,
      ...
      next_remito_number = case
        when public.companies.next_remito_number is null then excluded.next_remito_number
        when excluded.next_remito_number is null then null
        else greatest(public.companies.next_remito_number, excluded.next_remito_number)
      end,
      updated_at = now()
  ...
  for v_item in select * from jsonb_array_elements(v_locations) loop
    ...
    values (
      ...
      coalesce(nullif(v_item->>'code', ''), upper(v_slug || '_' || public.normalize_company_admin_slug(v_item->>'name'))),
      coalesce(nullif(v_item->>'slug', ''), public.normalize_company_admin_slug(v_item->>'name')),
      v_item->>'name',
    ...
    set company_id = excluded.company_id,
        ...
        schedule_flow = excluded.schedule_flow,
        updated_at = now();
  end loop;
