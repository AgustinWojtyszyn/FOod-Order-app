-- EPSE orders must let every EPSE user choose any active EPSE requesting location.
-- Delivery snapshots still resolve through order_locations.default_delivery_location_id.
...
declare
  ...
  v_is_epse_location boolean := false;
begin
  if v_uid is null then
    raise exception 'not_authenticated';
  end if;
  ...
  if v_location.id is not null then
    ...
    v_is_epse_location := upper(coalesce(v_organization.code, '')) = 'EPSE';

    if not public.is_admin() and not v_is_epse_location and not exists (
      select 1
...
begin
  ...
  if not public.is_admin() and not v_admin_extra_allowed and not v_is_epse_location then
    perform public.sync_authorized_order_locations_for_user(new.user_id);
  end if;

  if not public.is_admin() and not v_admin_extra_allowed and not v_is_epse_location and not exists (
    select 1
    from public.user_order_locations uol
