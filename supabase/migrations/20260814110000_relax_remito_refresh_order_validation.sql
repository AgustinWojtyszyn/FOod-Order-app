-- Allow refreshing an issued remito snapshot with the frontend-refreshed order set.
-- The identity checks still protect company/date/number/location and the function
-- verifies that p_order_ids matches the snapshot order IDs sent by the app.
...
begin
  ...
  if v_requested_order_ids <> v_snapshot_order_ids
    or v_requested_order_ids <> v_snapshot_source_order_ids then
