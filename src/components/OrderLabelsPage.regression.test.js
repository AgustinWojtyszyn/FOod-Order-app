...
describe('order labels print flow', () => {
  ...
  it('uses one canonical 64 x 32 mm page geometry', () => {
    ...
    expect(previewSource).toContain('Configurar impresión')
    expect(previewSource).toContain('Impresora esperada:')
    expect(previewSource).toContain('Horizontal')
    expect(previewSource).toContain('la aplicación no puede cambiarla')
  })
